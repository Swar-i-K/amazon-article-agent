...
product data, using Google's Gemini API **free tier** (no credit card / no
paid plan needed for the volume this agent uses — 5-10 articles/day).

Get a free key: https://aistudio.google.com/app/apikey
Set it as a GitHub Actions secret named GEMINI_API_KEY.
"""
...
- Return ONLY the HTML body content, nothing else, no commentary.
"""
    resp = model.generate_content(prompt)
    html = _extract_html(resp.text)
    title = _make_title(model, product.title, focus_keyword, kind="single")
...
"""
    resp = model.generate_content(prompt)
    html = _extract_html(resp.text)
    title = _make_title(model, names, focus_keyword, kind="vs")
    return {"title": title, "html": html}


def _make_title(model, subject: str, focus_keyword: str, kind: str) -> str:
    prompt = f"""
