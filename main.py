...
is "vs", and which ASINs go with it), then for each job:
  ...
  2. Generates an SEO article with the free Gemini API (llm_generator.py)
  3. Uploads the first product's image as featured image + creates a
     DRAFT post in WordPress (wp_publisher.py)
...
from wp_publisher import upload_featured_image, create_draft_post
...
def run():
    ...
    for idx, job in enumerate(jobs, start=1):
        ...
        # 3. Publish draft to WordPress
        try:
            media_id = None
            first_img = next((p.image_url for p in products if p.image_url), None)
            if first_img:
                media_id = upload_featured_image(first_img, filename=f"job{idx}")
            ...
            print(f"  DRAFT created: {post.get('link') or post.get('id')}")
            summary.append({
